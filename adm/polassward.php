...
$rqtc=$bdd->prepare("SELECT d.`cod_pol`,d.`sequence`,d.`ndat_eff`,d.`ndat_ech`,d.`pn`,d.`pt`,d.`etat`,s.`nom_sous`,s.`pnom_sous`,u.`agence` FROM `policew` as d,`souscripteurw` as s,`utilisateurs` as u WHERE s.`cod_sous`=d.`cod_sous` AND d.`cod_prod`='5' AND s.`id_user`=u.`id_user` AND $condition ORDER BY d.`cod_pol` DESC");
$rqtc->execute();
$nbe = $rqtc->rowCount();
$nbpage=ceil($nbe/7);
//Pointeur de page
$part=$page*7;
//requete à suivre
$rqt=$bdd->prepare("SELECT d.`cod_pol`,d.`sequence`,d.`ndat_eff`,d.`ndat_ech`,d.`pn`,d.`pt`,d.`etat`,s.`nom_sous`,s.`pnom_sous`,u.`agence` FROM `policew` as d,`souscripteurw` as s,`utilisateurs` as u WHERE s.`cod_sous`=d.`cod_sous` AND d.`cod_prod`='5' AND s.`id_user`=u.`id_user` AND $condition ORDER BY d.`cod_pol` DESC LIMIT $part ,7");
$rqt->execute();	
	
}else{
//Calcule du nombre de page 
$rqtc=$bdd->prepare("SELECT d.`cod_pol`,d.`sequence`,d.`ndat_eff`,d.`ndat_ech`,d.`pn`,d.`pt`,d.`etat`,s.`nom_sous`,s.`pnom_sous`,u.`agence` FROM `policew` as d,`souscripteurw` as s,`utilisateurs` as u WHERE s.`cod_sous`=d.`cod_sous` AND d.`cod_prod`='5' AND s.`id_user`=u.`id_user` ORDER BY d.`cod_pol` DESC");
$rqtc->execute();
$nbe = $rqtc->rowCount();
$nbpage=ceil($nbe/7);
//Pointeur de page
$part=$page*7;
//requete à suivre
$rqt=$bdd->prepare("SELECT d.`cod_pol`,d.`sequence`,d.`ndat_eff`,d.`ndat_ech`,d.`pn`,d.`pt`,d.`etat`,s.`nom_sous`,s.`pnom_sous`,u.`agence` FROM `policew` as d,`souscripteurw` as s,`utilisateurs` as u WHERE s.`cod_sous`=d.`cod_sous` AND d.`cod_prod`='5' AND s.`id_user`=u.`id_user` ORDER BY d.`cod_pol` DESC LIMIT $part ,7");
$rqt->execute();
$nb = $rqt->execute();
}
?>  
			 ...
			 <?php if($row_res['etat']==0){ ?>
				...
				<td><?php  echo $row_res['sequence']; ?></td>
				  ...
				  <a href="sortie/contrat/<?php echo crypte($row_res['cod_pol']) ?>" onClick="window.open(this.href, 'Devis', 'height=600, width=800, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, directories=no, status=no'); return(false);" title="Imprimer"><i CLASS="icon-print icon-2x" style="color:#0e90d2"/></a> &nbsp;&nbsp;&nbsp;
					  <?php if($row_res['etat']==0){ ?>
				  <a onClick="vavade('<?php echo $row_res['cod_pol'];?>','<?php echo $row_res['ndat_eff'];?>','polassward.php')" title="Avenants"><i CLASS="icon-edit icon-2x " style="color:#BA1E20" /></a>  &nbsp;&nbsp;&nbsp;
					  <?php } ?>
...
<script language="JavaScript">
	...
	function frechpade(){
		...
		$("#content").load('adm/polassward.php?rech='+rech+'&crit='+crit);
	}
	function frechpade2(){
		var rech='';
		var crit=2;
	...
	function fpagepade(page,nbpage){
		if(page >=0){
			if(page == nbpage){
				...
				if(rech!='')
					$("#content").load('adm/polassward.php?page='+page+'&rech='+rech+'&crit='+crit);
				else
					$("#content").load('adm/polassward.php?page='+page);
			}
...
/*	function afrechpwar(){
        ...
        $("#content").load('adm/polassward.php?rech='+rech);
	}
	function afpagepwar(page,nbpage){
		if(page >=0){
			if(page == nbpage){
				alert("Vous ete a la derniere page!");
			}else{$("#content").load('adm/polassward.php?page='+page);}
		}else{alert("Vous ete en premiere page !");}
	}	
	*/
</script>		