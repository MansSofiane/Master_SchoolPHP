...
//Calcule du nombre de page 
    $rqtc=$bdd->prepare("SELECT d.`cod_dev`,d.`dat_eff`,d.`dat_ech`,d.`pn`,d.`pt`,d.`bool`,s.`nom_sous`,s.`pnom_sous` FROM `devisw` as d,`souscripteurw` as s WHERE s.`cod_sous`=d.`cod_sous` AND d.`etat`='0' AND d.`cod_prod`='10' AND s.`id_user` in (select id_user from utilisateurs where agence='$agence')   AND $condition ORDER BY d.`cod_dev` DESC");
    $rqtc->execute();
    $nbe = $rqtc->rowCount();
    $nbpage=ceil($nbe/7);
//Pointeur de page
    $part=$page*7;
//requete � suivre
    $rqt=$bdd->prepare("SELECT d.`cod_dev`,d.`dat_eff`,d.`dat_ech`,d.`pn`,d.`pt`,d.`bool`,s.`nom_sous`,s.`pnom_sous` FROM `devisw` as d,`souscripteurw` as s WHERE s.`cod_sous`=d.`cod_sous` AND d.`etat`='0' AND d.`cod_prod`='10' AND s.`id_user` in (select id_user from utilisateurs where agence='$agence')   AND $condition ORDER BY d.`cod_dev` DESC LIMIT $part ,7");
    $rqt->execute();

}else{
//Calcule du nombre de page 
    $rqtc=$bdd->prepare("SELECT d.`cod_dev`,d.`dat_eff`,d.`dat_ech`,d.`pn`,d.`pt`,d.`bool`,s.`nom_sous`,s.`pnom_sous` FROM `devisw` as d,`souscripteurw` as s WHERE s.`cod_sous`=d.`cod_sous` AND d.`etat`='0' AND d.`cod_prod`='10' AND s.`id_user` in (select id_user from utilisateurs where agence='$agence')  ORDER BY d.`cod_dev` DESC");
    $rqtc->execute();
    $nbe = $rqtc->rowCount();
    $nbpage=ceil($nbe/7);
    //Pointeur de page
    $part=$page*7;
    //echo "<script type="."'text/JavaScript'"."> alert("."'id_user: $id_user => agence: $agence==>nbe: $nbe ==> nbpage:  $nbpage ==> part: $part'".");  </script>"; 

    //requete � suivre
    $rqt=$bdd->prepare("SELECT d.`cod_dev`,d.`dat_eff`,d.`dat_ech`,d.`pn`,d.`pt`,d.`bool`,s.`nom_sous`,s.`pnom_sous` FROM `devisw` as d,`souscripteurw` as s WHERE s.`cod_sous`=d.`cod_sous` AND d.`etat`='0' AND d.`cod_prod`='10' AND s.`id_user` in (select id_user from utilisateurs where agence='$agence')  ORDER BY d.`cod_dev` DESC LIMIT $part ,7");
    $rqt->execute();
    $nb = $rqt->execute();
}
?>
            ...
            while ($row_res=$rqt->fetch()){  ?>
                ...
                <tr class="gradeX">
                    ...
                    <td>&nbsp;
                        ...
                        <?php if($row_res['bool']==0){ ?>
                            <a onClick="vdev2('<?php echo $row_res['cod_dev'];?>','<?php echo $row_res['dat_eff'];?>','asspta.php')" title="Valider"><i CLASS="icon-ok icon-2x" style="color:green"/></a>&nbsp;&nbsp;&nbsp;
                        <?php } ?>
...
<script language="JavaScript">
    function frechdade(){
        ...
        $("#content").load('produit/asspta.php?rech='+rech+'&crit='+crit);
    }
    function frechdade2(){
        var rech='';
        var crit=2;
    ...
    function fpagedade(page,nbpage){
        if(page >=0){
            if(page == nbpage){
                ...
                if(rech!='')
                    $("#content").load('produit/asspta.php?page='+page+'&rech='+rech+'&crit='+crit);
                else
                    $("#content").load('produit/asspta.php?page='+page);
            }
