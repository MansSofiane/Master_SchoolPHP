...
$rqtc=$bdd->prepare("SELECT d.`cod_dev`,d.`dat_eff`,d.`dat_ech`,d.`pn`,d.`pt`,d.`bool`,s.`nom_sous`,s.`pnom_sous`,u.`agence` FROM `devisw` as d,`souscripteurw` as s,`utilisateurs` as u WHERE s.`cod_sous`=d.`cod_sous` AND d.`etat`='0' AND d.`bool`='1' AND d.`cod_prod`='2' AND d.`cod_formul`='1' AND s.`id_user`=u.`id_user` AND $condition ORDER BY d.`cod_dev` DESC");
$rqtc->execute();
$nbe = $rqtc->rowCount();
$nbpage=ceil($nbe/7);
//Pointeur de page
$part=$page*7;
//requete à suivre
$rqt=$bdd->prepare("SELECT d.`cod_dev`,d.`dat_eff`,d.`dat_ech`,d.`pn`,d.`pt`,d.`bool`,s.`nom_sous`,s.`pnom_sous`,u.`agence` FROM `devisw` as d,`souscripteurw` as s,`utilisateurs` as u WHERE s.`cod_sous`=d.`cod_sous` AND d.`etat`='0' AND d.`bool`='1' AND d.`cod_prod`='2' AND d.`cod_formul`='1' AND s.`id_user`=u.`id_user` AND $condition ORDER BY d.`cod_dev` DESC LIMIT $part ,7");
$rqt->execute();	
	
}else{
//Calcule du nombre de page 
$rqtc=$bdd->prepare("SELECT d.`cod_dev`,d.`dat_eff`,d.`dat_ech`,d.`pn`,d.`pt`,d.`bool`,s.`nom_sous`,s.`pnom_sous`,u.`agence` FROM `devisw` as d,`souscripteurw` as s,`utilisateurs` as u WHERE s.`cod_sous`=d.`cod_sous` AND d.`etat`='0' AND d.`bool`='1' AND d.`cod_prod`='2' AND d.`cod_formul`='1' AND s.`id_user`=u.`id_user` ORDER BY d.`cod_dev` DESC");
$rqtc->execute();
$nbe = $rqtc->rowCount();
$nbpage=ceil($nbe/7);
//Pointeur de page
$part=$page*7;
//requete à suivre
$rqt=$bdd->prepare("SELECT d.`cod_dev`,d.`dat_eff`,d.`dat_ech`,d.`pn`,d.`pt`,d.`bool`,s.`nom_sous`,s.`pnom_sous`,u.`agence` FROM `devisw` as d,`souscripteurw` as s,`utilisateurs` as u WHERE s.`cod_sous`=d.`cod_sous` AND d.`etat`='0' AND d.`bool`='1' AND d.`cod_prod`='2' AND d.`cod_formul`='1' AND s.`id_user`=u.`id_user` ORDER BY d.`cod_dev` DESC LIMIT $part ,7");
$rqt->execute();
$nb = $rqt->execute();
}
?>  
          ...
          <div class="widget-content nopadding">
			...
			<tr class="gradeX">
				...
				<td><?php echo $row_res['cod_dev']; ?></td>
			      ...
			      <a onClick="surprime('prod','assiaccind.php','<?php echo $row_res['cod_dev'];?>','<?php echo $row_res['pn'];?>')" title="Accorder"><i CLASS="icon-thumbs-up icon-2x" style="color:green"/></a>&nbsp;&nbsp;&nbsp;
				  <a onClick="rdev('prod','assiaccind.php','<?php echo $row_res['cod_dev'];?>')" title="Rejeter"><i CLASS="icon-thumbs-down icon-2x" style="color:red"/></a>
				  </td>
                </tr>
			<?php } ?>
              </tbody>
...
<script language="JavaScript">
	...
	function frechpade(){
		...
		$("#content").load('adm/assiaccind.php?rech='+rech+'&crit='+crit);
	}
	function frechpade2(){
		var rech='';
		var crit=2;
	...
	function fpagepade(page,nbpage){
		if(page >=0){
			if(page == nbpage){
				...
				if(rech!='')
					$("#content").load('adm/assiaccind.php?page='+page+'&rech='+rech+'&crit='+crit);
				else
					$("#content").load('adm/assiaccind.php?page='+page);
			}
		}else{alert("Vous ete en premiere page !");}
	}
	/*
	function afrechdiacc(){
		var rech=document.getElementById("ansousdiacc").value;
        $("#content").load('adm/assiaccind.php?rech='+rech);
	}
	function afpagediacc(page,nbpage){
		if(page >=0){
			if(page == nbpage){
				alert("Vous ete a la derniere page!");
			}else{$("#content").load('adm/assiaccind.php?page='+page);}
		}else{alert("Vous ete en premiere page !");}
	}
			*/
</script>		