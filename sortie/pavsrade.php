...
$pdf->Cell(190,8,'Assurance Décès Emprunteur','0','0','C');$pdf->Ln();
    $cod_pol=$row_g['cod_pol'];
    $rqtu=$bdd->prepare("select * from utilisateurs where  id_user = (select id_user from souscripteurw where cod_sous=(select cod_sous from policew where cod_pol='".$cod_pol."'));");
    $rqtu->execute();
while ($row_user=$rqtu->fetch()){
$pdf->Cell(190,8,"Avenant d'Annulation Sans Ristourne",'0','0','L');$pdf->Ln();
...
$pdf->SetFont('Arial','B',10);
$pdf->Cell(30,5,"Le montant à ristourner en lettres",'0','0','L');$pdf->Ln();
$pdf->SetFont('Arial','B',12);$pdf->SetFillColor(255,255,255);
$pdf->MultiCell(190,12,"".$somme."",1,'C',true);

$pdf->Ln(9);
$pdf->SetFont('Arial','I',6);
$pdf->Cell(0,6,"« Il n’est pas autrement dérogé aux autres clauses et conditions tant générales que particulières de la police d’assurance à laquelle le présent avenant demeure annexé pour en faire partie intégrante.",0,0,'C');
