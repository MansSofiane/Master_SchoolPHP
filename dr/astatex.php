...
$rqt = $bdd->prepare("SELECT id_user,agence FROM `utilisateurs`  WHERE id_par='$id_user' ORDER BY `agence`");
$rqt->execute();
?>
...
<div id="breadcrumb"> <a class="tip-bottom"><i class="icon-signal"></i> Statistiques</a><a class="current">Excel</a></div>
  ...
  <div class="widget-box">
            ...
            <ul class="quick-actions">
			  ...
			  <li class="bg_lo"> <a onClick="dprod('5')"> <i class="icon-bar-chart"></i>C-Sein (Warda)</a> </li>
			</ul>
</div>
...
function dprod(code) {
	...
	if (verifdatestat(date1) && verifdatestat(date2)) {
		...
		window.open('excel/Excel-D/' + user + '/' + d1 + '/' + d2+ '/' + code,"Generation Excel","menubar=no, status=no, scrollbars=yes, menubar=no, width=600, height=230");
