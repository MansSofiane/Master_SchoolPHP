...
$result="";
	if(isset($_POST['submit'])) {
		...
		$upload = new Upload('/CASH/produit/sinistre/files', $_FILES['file']);
		//$upload = new Upload($strn, $_FILES['file']);
		$name=$_SESSION['agencesal'];


		$name=$name.'-'.$datesys.$_FILES['file']['name'];
		echo '$name';
		...
		//$rqt=$bdd->prepare("INSERT INTO `document`(`id_doc`, `chemin`, `dat_doc`, `id_user`) VALUES ('','$chemin','$datesys','$id_user')");
		//$rqt->execute();
