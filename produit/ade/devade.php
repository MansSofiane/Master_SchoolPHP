...
<div id="content-header">
  ...
  <div class="row-fluid">  
    <div class="span12">
      ...
      <div id="breadcrumb"> <a class="current"><i></i>Souscripteur</a><a>Assure</a><a>Beneficiaire</a><a>Capital</a><a>Selection-Medical</a><a>Validation</a></div>
        <div class="widget-content nopadding">
          <form class="form-horizontal">
            ...
            <div class="form-actions" align="right">
			  <input  type="button" class="btn btn-success" onClick="instarsous('<?php echo $id_user; ?>','<?php echo $tokade1; ?>')" value="Suivant" />
...
<script language="JavaScript">
    ...
    function calageade(dd)
    {

        var bb1=document.getElementById("datsys");
        var aa=new Date(dfrtoen_ade(dd.value));
    ...
    function instarsous(user,tok) {
        ...
        if (civilite && nom && prenom && adr && reponse) {
            ...
            if (verifdateade(datnais)) {
                age = calageade(datnais);
                date2 = dfrtoen_ade(datnais.value);
                if (age >= 18) {
                    if (reponse == 1) {
                        xhr.open("GET", "produit/ade/nsous.php?civs=" + civilite + "&noms=" + nom + "&prenoms=" + prenom + "&adrs=" + adr + "&ages=" + age + "&dnaiss=" + date2 + "&mails=" + mail + "&tels=" + tel + "&rps=" + reponse + "&tok=" + tok, false);
                        xhr.send(null);
                        $("#content").load("produit/ade/devade1.php?tok=" + tok);
                    } else {
                        xhr.open("GET", "produit/ade/nsous.php?civs=" + civilite + "&noms=" + nom + "&prenoms=" + prenom + "&adrs=" + adr + "&ages=" + age + "&dnaiss=" + date2 + "&mails=" + mail + "&tels=" + tel + "&rps=" + reponse + "&tok=" + tok, false);
                        xhr.send(null);
                        $("#content").load("produit/ade/devade2.php?tok=" + tok);
                    }
