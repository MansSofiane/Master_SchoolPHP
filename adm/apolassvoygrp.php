...
as d,`souscripteurw` as s WHERE s.`cod_sous`=d.`cod_sous`  AND d.`cod_prod`='1'  and d.cod_formul='5' AND $condition ORDER BY d.`cod_pol` DESC");
    $rqtc->execute();
    $nbe = $rqtc->rowCount();
    $nbpage=ceil($nbe/7);
//Pointeur de page
...
as d,`souscripteurw` as s WHERE s.`cod_sous`=d.`cod_sous` AND d.`cod_prod`='1'  and d.cod_formul='5' AND $condition ORDER BY d.`cod_pol` DESC LIMIT $part ,7");
    $rqt->execute();

}else{
//Calcule du nombre de page 
    $rqtc=$bdd->prepare("SELECT d.`cod_pol`,d.`sequence`,d.`ndat_eff`,d.`ndat_ech`,d.`pn`,d.`pt`,d.`etat`,s.`nom_sous`,s.`pnom_sous`,s.`rp_sous` FROM `policew`
as d,`souscripteurw` as s WHERE s.`cod_sous`=d.`cod_sous` AND d.`cod_prod`='1'  and d.cod_formul='5' ORDER BY d.`cod_pol` DESC");
    $rqtc->execute();
    $nbe = $rqtc->rowCount();
    $nbpage=ceil($nbe/7);
//Pointeur de page
...
//requete � suivre
    $rqt=$bdd->prepare("SELECT d.`cod_pol`,d.`sequence`,d.`ndat_eff`,d.`ndat_ech`,d.`pn`,d.`pt`,d.`etat`,s.`nom_sous`,s.`pnom_sous`,s.`rp_sous` FROM `policew`
as d,`souscripteurw` as s WHERE s.`cod_sous`=d.`cod_sous` AND d.`cod_prod`='1'  and d.cod_formul='5' ORDER BY d.`cod_pol` DESC LIMIT $part ,7");
    $rqt->execute();
    $nb = $rqt->execute();
}
?>
            ...
            while ($row_res=$rqt->fetch()){  ?>
                ...
                <tr class="gradeX">
                    ...
                    <td>&nbsp;
                        ...
                        <?php if($row_res['etat']==0){ ?>
                        <a onClick="vavvoy('<?php echo $row_res['cod_pol'];?>','<?php echo $row_res['ndat_eff'];?>','apolassvoygrp.php')" title="Avenant"><i CLASS="icon-edit icon-2x " style="color:#BA1E20" /></a>
                        <?php } ?>
...
<script language="JavaScript">
    ...
    function frechpade(){
        ...
        $("#content").load('adm/apolassvoygrp.php?rech='+rech+'&crit='+crit);
    }
    function frechpade2(){
        var rech='';
        var crit=2;
    ...
    function fpagepade(page,nbpage){
        if(page >=0){
            if(page == nbpage){
                ...
                if(rech!='')
                    $("#content").load('adm/apolassvoygrp.php?page='+page+'&rech='+rech+'&crit='+crit);
                else
                    $("#content").load('adm/apolassvoygrp.php?page='+page);
            }
  ...
  /*  function afrechvgrp(){
        ...
        $("#content").load('adm/apolassvoygrp.php?rech='+rech);
    }
    function afpagevgrp(page,nbpage){
        if(page >=0){
            if(page == nbpage){
                alert("Vous ete a la derniere page!");
            }else{$("#content").load('adm/apolassvoygrp.php?page='+page);}
        }else{alert("Vous ete en premiere page !");}
    }
    */
</script>