...
$rqtc=$bdd->prepare("SELECT * FROM `policew` as p,`demande` as d,`utilisateurs` as u, document as doc WHERE p.cod_pol = d.cod_pol and d.id_user = u.id_user and doc.id_demande = d.id_demande and p.cod_pol=$code order by d.id_demande  DESC  ");
$rqtc->execute();
$nbe = $rqtc->rowCount();
$nbpage=ceil($nbe/7);
...
<div id="content-header">
    <div id="breadcrumb"> <a class="tip-bottom"><i class="icon-home"></i> Produit</a><a class="current">Assurance-Voyage-individuel  > demande </a> </div>
</div>
            ...
            while ($row_res=$rqtc->fetch()){  ?>
                ...
                <tr class="gradeX">
                   ...
                   <?php }
                    ...
                    if($row_res['etat_dem']!=1){
                    ?>
                    <a onClick="accorder('prod','dempolvoyind.php','<?php echo $row_res['id_demande']; ?>', '<?php echo $row_res['cod_pol']; ?>',' <?php echo $row_res['sequence'] ?>')" title="Accorder"><i CLASS="icon-thumbs-up icon-2x" style="color:green"/></a>&nbsp;&nbsp;&nbsp;
                    <?php } 
                     if($row_res['etat_dem']!=2){
                    ?>
				    <a onClick="rejeter('prod','dempolvoyind.php','<?php echo $row_res['id_demande']; ?>', '<?php echo $row_res['cod_pol']; ?>',' <?php echo $row_res['sequence'] ?>')" title="Rejeter"><i CLASS="icon-thumbs-down icon-2x" style="color:red"/></a>
                    <?php } ?>    
                </td>
                </tr>
            <?php } ?>
