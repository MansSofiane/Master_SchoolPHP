...
$rqtc=$bdd->prepare("SELECT d.`cod_pol`,d.`sequence`,d.`ndat_eff`,d.`ndat_ech`,d.`pn`,d.`pt`,d.`etat`,s.`nom_sous`,s.`pnom_sous`,u.`agence` FROM `policew` as d,`souscripteurw` as s,`utilisateurs` as u WHERE s.`cod_sous`=d.`cod_sous` AND d.`cod_prod`='2' AND d.`cod_formul`='2' AND s.`id_user`=u.`id_user` AND u.id_par='$id_user' AND $condition ORDER BY d.`cod_pol` DESC");
$rqtc->execute();
$nbe = $rqtc->rowCount();
$nbpage=ceil($nbe/7);
//Pointeur de page
$part=$page*7;
//requete à suivre
$rqt=$bdd->prepare("SELECT d.`cod_pol`,d.`sequence`,d.`ndat_eff`,d.`ndat_ech`,d.`pn`,d.`pt`,d.`etat`,s.`nom_sous`,s.`pnom_sous`,u.`agence` FROM `policew` as d,`souscripteurw` as s,`utilisateurs` as u WHERE s.`cod_sous`=d.`cod_sous` AND d.`cod_prod`='2' AND d.`cod_formul`='2' AND s.`id_user`=u.`id_user` AND u.id_par='$id_user' AND $condition ORDER BY d.`cod_pol` DESC LIMIT $part ,7");
$rqt->execute();	
	
}else{
//Calcule du nombre de page 
$rqtc=$bdd->prepare("SELECT d.`cod_pol`,d.`sequence`,d.`ndat_eff`,d.`ndat_ech`,d.`pn`,d.`pt`,d.`etat`,s.`nom_sous`,s.`pnom_sous`,u.`agence` FROM `policew` as d,`souscripteurw` as s,`utilisateurs` as u WHERE s.`cod_sous`=d.`cod_sous` AND d.`cod_prod`='2' AND d.`cod_formul`='2' AND s.`id_user`=u.`id_user` AND u.id_par='$id_user' ORDER BY d.`cod_pol` DESC");
$rqtc->execute();
$nbe = $rqtc->rowCount();
$nbpage=ceil($nbe/7);
//Pointeur de page
$part=$page*7;
//requete à suivre
$rqt=$bdd->prepare("SELECT d.`cod_pol`,d.`sequence`,d.`ndat_eff`,d.`ndat_ech`,d.`pn`,d.`pt`,d.`etat`,s.`nom_sous`,s.`pnom_sous`,u.`agence` FROM `policew` as d,`souscripteurw` as s,`utilisateurs` as u WHERE s.`cod_sous`=d.`cod_sous` AND d.`cod_prod`='2' AND d.`cod_formul`='2' AND s.`id_user`=u.`id_user` AND u.id_par='$id_user' ORDER BY d.`cod_pol` DESC LIMIT $part ,7");
$rqt->execute();
$nb = $rqt->execute();
}
?>  
 ...
 <div id="content-header">
  ...
  <div class="widget-box">
            <ul class="quick-actions">
	          ...
	          <li class="bg_lv"> <a onClick="dMenu1('prod','assiacc.php')"> <i class="icon-backward"></i>Precedent</a></li>
	        </ul>
  </div>
   <div class="widget-box">
          ...
          <div class="widget-content nopadding">
			...
			<tr class="gradeX">
				  ...
				  <a href="sortie/contrat5/<?php echo crypte($row_res['cod_pol']) ?>" onClick="window.open(this.href, 'Devis', 'height=600, width=800, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, directories=no, status=no'); return(false);" title="Imprimer"><i CLASS="icon-print icon-2x" style="color:#0e90d2"/></a>
					  &nbsp;&nbsp;&nbsp;
					  <a onClick="dlav('<?php echo $row_res['cod_pol'];?>','avpolassiaccgrp.php')" title="Liste-Avenants"><i CLASS="icon-list-alt icon-2x" style="color:black"/></a>
...
<script language="JavaScript">
	function frechpade(){
		...
		$("#content").load('dr/polassiaccgrp.php?rech='+rech+'&crit='+crit);
	}
	function frechpade2(){
		var rech='';
		var crit=2;
	...
	function fpagepade(page,nbpage){
		if(page >=0){
			if(page == nbpage){
				...
				if(rech!='')
					$("#content").load('dr/polassiaccgrp.php?page='+page+'&rech='+rech+'&crit='+crit);
				else
					$("#content").load('dr/polassiaccgrp.php?page='+page);
			}
	...
	/*	function afrechpiaccgd(){
            var rech=document.getElementById("ansouspiaccg").value;
            $("#content").load('dr/polassiaccgrp.php?rech='+rech);
        }
        function afpagepiaccgd(page,nbpage){
            if(page >=0){
                if(page == nbpage){
                    alert("Vous ete a la derniere page!");
                }else{$("#content").load('dr/polassiaccgrp.php?page='+page);}
            }else{alert("Vous ete en premiere page !");}
        }		*/
</script>		