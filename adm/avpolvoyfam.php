...
<div class="widget-box">
    <ul class="quick-actions">
        ...
        <li class="bg_lv"> <a onClick="aMenu1('prod','apolassvoyfam.php')"> <i class="icon-backward"></i>Precedent</a></li>
        <li class="bg_lg"> <a onClick="aMenu1('prod','apolassvoyfam.php')"> <i class="icon-folder-open"></i>Visualiser-Contrats</a> </li>
        ...
        <table class="table table-bordered data-table">
            ...
            while ($row_res=$rqt->fetch()){  ?>
                ...
                <tr class="gradeX">
                    ...
                    <td>&nbsp;
                        <?php if($row_res['lib_mpay']==74){ ?>
                            <a href="sortie/f-avenant/<?php echo crypte($row_res['cod_av']) ?>" onClick="window.open(this.href, 'Devis', 'height=600, width=800, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, directories=no, status=no'); return(false);" title="Imprimer"><i CLASS="icon-print icon-2x" style="color:#0e90d2"/></a>
                        <?php }
						   if($row_res['lib_mpay']==14){ ?>
                        ...
                        <?php }  if($row_res['lib_mpay']==70){ ?>
                            <a href="sortie/f-avenantp/<?php echo crypte($row_res['cod_av']) ?>" onClick="window.open(this.href, 'Devis', 'height=600, width=800, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, directories=no, status=no'); return(false);" title="Imprimer"><i CLASS="icon-print icon-2x" style="color:#0e90d2"/></a>
                        <?php }
                        ?>
                    </td>
                </tr>
...
<script language="JavaScript">
    function frechapconso(){
        ...
        $("#content").load('adm/avpolvoyfam.php?rech='+rech);
    }
    function fpageapconso(page,nbpage,code){
        if(page >=0){
            if(page == nbpage){
                alert("Vous ete a la derniere page!");
            }else{$("#content").load('adm/avpolvoyfam.php?page='+page+'&code='+code);}
