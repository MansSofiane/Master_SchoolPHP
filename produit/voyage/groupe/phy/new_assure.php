...
//calcule age.
        ...
        try {
            if(isset($sexe) && isset($nom) && isset($pnom) && isset($pass) && isset($datenai)  ) {
                $rqtsous3 = $bdd->prepare("INSERT INTO  `souscripteurw`(`cod_sous`,`nom_sous`, `pnom_sous`, `passport`, `datedpass`,`mail_sous`,`tel_sous`,`adr_sous`, `dnais_sous`,`age`, `civ_sous`,`cod_par`, `id_user`) VALUES ('','$nom','$pnom','$pass','$datedpass','$mail','$tel','$adr','$datenai','$age_ass','$sexe','$codsous','$id_user')");
                $rqtsous3->execute();
            }
        } catch (Exception $ex) {
