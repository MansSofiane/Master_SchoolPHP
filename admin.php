<?php session_start();
if ($_SESSION['loginsal'] && $_SESSION['Role'] == "admin"){
}
else {
...
<div id="user-nav" class="navbar navbar-inverse">
  <ul class="nav">
   <!--
    ...
    <li  class="dropdown" id="profile-messages2" ><a data-toggle="dropdown" data-target="#profile-messages2" class="dropdown-toggle"><i class="icon icon-user"></i>  <span class="text">Bienvenue- <?php echo $_SESSION['nomsal']?></span><b class="caret"></b></a>
      <ul class="dropdown-menu">
        <li><a onClick="Menu('macc','php/cmpt/acmpt.php')"><i class="icon-user"></i> Compte</a></li>
          <li class="divider"></li>
...
<div id="sidebar">
   <ul><input type="hidden" id="datsys" value="<?php echo $datesys; ?>"/>
    <li  id="macc" class="active"><a onClick="aMenu1('macc','../adash.php')"><i class="icon icon-home"></i> <span>Acceuil</span></a> </li>
	<!-- <li id="param" class="submenu"> <a><i class="icon icon-cog"></i> <span>Parametres</span> <span class="label label-important">3</span></a>
	 ...
	 <li id="prod" class="submenu"> <a><i class="icon icon-th-list"></i> <span>Produits-Assurance</span> <span class="label label-important">5</span></a>
      <ul>
        <li><a onClick="aMenu1('prod','assvoy.php')">&nbsp;&nbsp;&nbsp;&nbsp;Assurance-Voyage</a></li>
		<li><a onClick="aMenu1('prod','asstd.php')">&nbsp;&nbsp;&nbsp;&nbsp;Temporaire-Deces</a></li>
		<li><a onClick="aMenu1('prod','asscim.php')">&nbsp;&nbsp;&nbsp;&nbsp;A-Deces-Emprunteur</a></li>
		<li><a onClick="aMenu1('prod','assiacc.php')">&nbsp;&nbsp;&nbsp;&nbsp;Individuel-Accident</a></li>
       <!-- <li><a onClick="Menu1('prod','assccon.php')">&nbsp;&nbsp;&nbsp;&nbsp;ADE-consommation</a></li>-->
		<li><a onClick="aMenu1('prod','assward.php')">&nbsp;&nbsp;&nbsp;&nbsp;Cancer du Sein Warda</a></li>
      </ul>
    </li>
	<!--  
	 <li id="mclt" class="submenu" class="submenu" >  <a ><i class="icon icon-user"></i> <span>Clients</span><span class="label label-important"></span><span class="label label-important">2</span></a> 
	   <ul>
        <li><a onClick="Menu3('mclt','souscripteur.php')">&nbsp;&nbsp;&nbsp;&nbsp;Souscripteurs</a></li>
        <li><a onClick="Menu3('mclt','assure.php')">&nbsp;&nbsp;&nbsp;&nbsp;Assures</a></li>
      </ul>
	  </li>   
    ...
    <li id="mged"><a onClick="Menu1('mged','ged.html')"><i class="icon icon-file"></i> <span>Documents</span></a></li>-->
	   ...
	   <ul>
        <li> <a onClick="aMenu1('mstat','stat.php')"><span>Etat-PDF</span></a> </li> 
	    <li> <a onClick="aMenu1('mstat','astatex.php')"><span>Etat-EXCEL</span></a> </li> 
      </ul>
	  </li> 
...
<!--main-container-part-->
<div id="content">
<script>$("#content").load('adash.php');</script>
</div>
</body>
</html>
...
function aMenu1(id,page) {
document.getElementById('macc').setAttribute("class", "hover");
document.getElementById('mstat').setAttribute("class", "hover");
document.getElementById('prod').setAttribute("class", "hover");
document.getElementById(id).setAttribute("class", "active");
$("#content").load('adm/'+page);
}
function aform(page) {
...
function accorder(id,page,id_demande, codPol, sequence = 0)
{
    var ok = confirm("Confirmez la demande d annulation");
    if (ok)
    {
        $("#content").load("adm/acorder.php?id="+id+"&page="+page+"&id_demande=" + id_demande+"&codpol="+codPol );
    }
    $("#content").load("adm/"+page+"?code="+codPol);
}
function rejeter(id,page,id_demande, codPol, sequence)
{
    var ok = confirm("Confirmez le refus  de la demande  ?");
    if (ok)
    {
        $("#content").load("adm/rejter.php?id="+id+"&page="+page+"&id_demande=" + id_demande+"&codpol="+codPol );
    }
    $("#content").load("adm/"+page+"?code="+codPol);
}
...
function rdev(id,page,codedev){
 ...
 if (window.XMLHttpRequest) { 
     ...
     var ok=confirm("Comfirmez Rejet du devis ");
	 if (ok){   
...
function alav(codepol,page){ 
	  $("#content").load('adm/'+page+'?code='+codepol);
	}
...
function vavade(codepol,date,page){
    ...
    if (ok){
        $("#content").load('php/adavenant/fav.php?code='+codepol+'&page='+page);
...
function vavvoy(codepol,date,page){
    ...
    else if (window.ActiveXObject)
    {
        ...
        var ok=confirm("Comfirmez la Souscription-Avenant");
        if (ok){
            $("#content").load('php/adavenant/voy/fav.php?code='+codepol+'&page='+page);
