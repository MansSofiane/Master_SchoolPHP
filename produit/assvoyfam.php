...
$condition AND p.`cod_per`=d.`cod_per` AND d.`cod_formul`='4' ORDER BY d.`cod_dev` DESC");
    $rqtc->execute();
    $nbe = $rqtc->rowCount();
    $nbpage=ceil($nbe/7);
//Pointeur de page
...
`devisw` as d,`souscripteurw` as s, `periode` as p WHERE s.`cod_sous`=d.`cod_sous` AND d.`etat`='0' AND d.`cod_prod`='1'  AND s.`id_user`='$id_user' AND
$condition AND p.`cod_per`=d.`cod_per` AND d.`cod_formul`='4' ORDER BY d.`cod_dev` DESC LIMIT $part ,7");
    $rqt->execute();

}else {
//Calcule du nombre de page
    $rqtc = $bdd->prepare("SELECT d.`cod_dev`,d.`cod_per`,d.`dat_eff`,d.`dat_ech`,d.`pn`,d.`pt`,d.`bool`,s.`nom_sous`,s.`pnom_sous`,p.`lib_per`,s.`rp_sous` FROM
`devisw` as d,`souscripteurw` as s, `periode` as p WHERE s.`cod_sous`=d.`cod_sous` AND d.`etat`='0' AND d.`cod_prod`='1'  AND s.`id_user`='$id_user' AND p.`cod_per`=d.`cod_per` AND d.`cod_formul`='4' ORDER BY
d.`cod_dev` DESC");
    $rqtc->execute();
    $nbe = $rqtc->rowCount();
    $nbpage = ceil($nbe / 7);
...
d.`cod_dev` AND d.`cod_formul`='4' DESC LIMIT $part ,7");
    $rqtt->execute();
}
?>  
...
<script language="JavaScript">
  ...
  /*  function frechdade(){
        ...
        $("#content").load('produit/assvoyfam.php?rech='+rech);
    }
    function fpagedade(page,nbpage){
        if(page >=0){
            if(page == nbpage){
                alert("Vous ete a la derniere page!");
            }else{$("#content").load('produit/assvoyfam.php?page='+page);}
        }else{alert("Vous ete en premiere page !");}
    }
    */
    function ndev(){
