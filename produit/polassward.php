...
$rqtc=$bdd->prepare("SELECT d.`cod_pol`,d.sequence,d.`ndat_eff`,d.`ndat_ech`,d.`pn`,d.`pt`,d.`etat`,s.`nom_sous`,s.`pnom_sous` FROM `policew` as d,`souscripteurw` as s WHERE s.`cod_sous`=d.`cod_sous` AND d.`cod_prod`='5' AND s.`id_user`='$id_user' AND $condition ORDER BY d.`cod_pol` DESC");
$rqtc->execute();
$nbe = $rqtc->rowCount();
$nbpage=ceil($nbe/7);
//Pointeur de page
$part=$page*7;
//requete à suivre
$rqt=$bdd->prepare("SELECT d.`cod_pol`,d.sequence,d.`ndat_eff`,d.`ndat_ech`,d.`pn`,d.`pt`,d.`etat`,s.`nom_sous`,s.`pnom_sous` FROM `policew` as d,`souscripteurw` as s WHERE s.`cod_sous`=d.`cod_sous` AND d.`cod_prod`='5' AND s.`id_user`='$id_user' AND $condition ORDER BY d.`cod_pol` DESC LIMIT $part ,7");
$rqt->execute();	
	
}else{
//Calcule du nombre de page 
$rqtc=$bdd->prepare("SELECT d.`cod_pol`,d.sequence,d.`ndat_eff`,d.`ndat_ech`,d.`pn`,d.`pt`,d.`etat`,s.`nom_sous`,s.`pnom_sous` FROM `policew` as d,`souscripteurw` as s WHERE s.`cod_sous`=d.`cod_sous` AND d.`cod_prod`='5' AND s.`id_user`='$id_user' ORDER BY d.`cod_pol` DESC");
$rqtc->execute();
$nbe = $rqtc->rowCount();
$nbpage=ceil($nbe/7);
//Pointeur de page
$part=$page*7;
//requete à suivre
$rqt=$bdd->prepare("SELECT d.`cod_pol`,d.sequence,d.`ndat_eff`,d.`ndat_ech`,d.`pn`,d.`pt`,d.`etat`,s.`nom_sous`,s.`pnom_sous` FROM `policew` as d,`souscripteurw` as s WHERE s.`cod_sous`=d.`cod_sous` AND d.`cod_prod`='5' AND s.`id_user`='$id_user' ORDER BY d.`cod_pol` DESC LIMIT $part ,7");
$rqt->execute();
$nb = $rqt->execute();
}
?>  
          ...
          <div class="widget-content nopadding">
			...
			<tr class="gradeX">
			 <?php if($row_res['etat']==0){ ?>
				 ...
				 <?php if($row_res['etat']==0){ ?> 
				  &nbsp;&nbsp;&nbsp;
		 <a onClick="vav('<?php echo $row_res['cod_pol'];?>','<?php echo $row_res['ndat_eff'];?>','polassward.php')" title="Avenants"><i CLASS="icon-edit icon-2x " style="color:#BA1E20" /></a>
		 <?php } ?>
		 &nbsp;&nbsp;&nbsp;
		 <a onClick="lav('<?php echo $row_res['cod_pol'];?>','avpolassward.php')" title="Liste-Avenants"><i CLASS="icon-list-alt icon-2x" style="color:black"/></a>
...
<script language="JavaScript">
	...
	function frechpade(){
		...
		$("#content").load('produit/polassward.php?rech='+rech+'&crit='+crit);
	}
	function frechpade2(){
		var rech='';
		var crit=2;
	...
	function fpagepade(page,nbpage){
		if(page >=0){
			if(page == nbpage){
				...
				if(rech!='')
					$("#content").load('produit/polassward.php?page='+page+'&rech='+rech+'&crit='+crit);
				else
					$("#content").load('produit/polassward.php?page='+page);
			}
		}else{alert("Vous ete en premiere page !");}
	}
	/*
	function frechpwar(){
		var rech=document.getElementById("nsouspwar").value;
        $("#content").load('produit/polassward.php?rech='+rech);
	}
	function fpagepwar(page,nbpage){
		if(page >=0){
			if(page == nbpage){
				alert("Vous ete a la derniere page!");
			}else{$("#content").load('produit/polassward.php?page='+page);}
		}else{alert("Vous ete en premiere page !");}
	}
	*/
