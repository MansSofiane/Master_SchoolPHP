			...
			<tr class="gradeX">
			     <?php if($row_res['etat']==0){ ?>
				  ...
				  <a onClick="vavade('<?php echo $row_res['cod_pol'];?>','<?php echo $row_res['ndat_eff'];?>','polasscim.php')" title="Avenants"><i CLASS="icon-edit icon-2x " style="color:#BA1E20" /></a> &nbsp;&nbsp;&nbsp;
					  <?php } ?>
		          <a onClick="alav('<?php echo $row_res['cod_pol'];?>','avpolasscim.php')" title="Liste-Avenants"><i CLASS="icon-list-alt icon-2x" style="color:black"/></a>
				  </td>
                </tr>
			<?php } ?>
              </tbody>
...
<script language="JavaScript">
	function frechpade(){
		...
		$("#content").load('adm/polasscim.php?rech='+rech+'&crit='+crit);
	}
	function frechpade2(){
		var rech='';
		var crit=2;
	...
	function fpagepade(page,nbpage){
		if(page >=0){
			if(page == nbpage){
				...
				if(rech!='')
					$("#content").load('adm/polasscim.php?page='+page+'&rech='+rech+'&crit='+crit);
				else
					$("#content").load('adm/polasscim.php?page='+page);
			}
	...
	function afrechpade(){
        ...
        $("#content").load('adm/polasscim.php?rech='+rech);
	}
	function afpagepade(page,nbpage){
		if(page >=0){
			if(page == nbpage){
				alert("Vous ete a la derniere page!");
			}else{$("#content").load('adm/polasscim.php?page='+page);}
		}else{alert("Vous ete en premiere page !");}
	}	
*/
</script>		