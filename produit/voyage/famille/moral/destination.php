...
if (  isset ($_REQUEST['codsous'])  && isset ($_REQUEST['nb_assur'])  ) {
...
<div class="row-fluid">
                    ...
                    <div class="control-group">
                        <div class="controls">
                            ...
                            <select id="opt">
                                ...
                                $rqtopt = $bdd->prepare("SELECT * FROM `option` WHERE cod_opt <= '22' AND cod_opt >= '22' ");//OR cod_opt = '25'
                                //$res_rqtop=mysql_query($rqtop) or die ($rqtop . "-----" . mysql_error());
                                $rqtopt->execute();
    ...
    <script language="JavaScript">
        ...
        function inserdevis(codsous) {
            ...
            var nb_assur="<?php echo $nb_assur;?>";
            var option = document.getElementById("opt").value;
            var pay = document.getElementById("pays").value;
            var dateffet = document.getElementById("dateeffet");
            var datech = document.getElementById("datecheance");
                    ...
                    if (compdat(dateffet) && compdat(datech)) {
                        if (compare_deux_dates(dateffet, datech))
                        {
                            ...
                            if(nb_jour<=365) {
                                ...
                                xhr.open("GET", "produit/voyage/famille/moral/devi.php?cod=" + codsous + "&option=" + option + "&duree=" + nb_jour + "&pay=" + pay + "&dateffet=" + date + "&datech=" + date2+"&nb_assur="+nb_assur,false);
                                xhr.send(null);
                                var rp=xhr.responseText;
                                if(rp==1){alert('insertion reussi '); $("#content").load("produit/assvoyfam.php"); }else{alert('Cas non assure !');}
