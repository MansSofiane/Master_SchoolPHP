...
<script language="JavaScript">
	...
	function choix_Demande(cod_police,page, id_user, datesys)
	{
		...
		if(type_av==5)//avenant Avec ristourne
		{
			...
			xhr.open("GET", "php/adavenant/voy/validationDem.php?code=" + cod_police + "&type=" + type_annul+"&date="+datesys + "&motif=" + motif + "&chemin=/Intranet-Salama-test/php/adavenant/voy/file/documents/ &etat=0 &desc="+desc+"&iduser=" + id_user, false);
			xhr.send(null);
		...
		if(type_av==4)
		{
			...
			xhr.open("GET", "php/adavenant/voy/validationDem.php?code=" + cod_police + "&type=" + type_annul+"&date="+datesys + "&motif=" + motif + "&chemin=SR&etat=0 &desc="+desc+"&iduser=" + id_user, false);
			xhr.send(null);
			alert("La demande a été créé avec succes");
			Menu1('prod', page);
	...
	function charger()
    {
        ...
        window.open('php/adavenant/voy/charger.php?code='+code , 'Chargement', 'height=600, width=800, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, directories=no, status=no'); return(false);
		document.getElementById('suiv').disabled=false;
    }
	function activerbtn()
    {
