			...
			<tr class="gradeX">
			 <?php if($row_res['etat']==0){ ?>
				  ...
				  <a href="sortie/contrat/<?php echo crypte($row_res['cod_pol']) ?>" onClick="window.open(this.href, 'Devis', 'height=600, width=800, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, directories=no, status=no'); return(false);" title="Imprimer"><i CLASS="icon-print icon-2x" style="color:#0e90d2"/></a>
					  &nbsp;&nbsp;&nbsp;
					  <a onClick="slav('<?php echo $row_res['cod_pol'];?>','avpolassward.php')" title="Liste-Avenants"><i CLASS="icon-list-alt icon-2x" style="color:black"/></a>
				  </td>
                </tr>
			<?php } ?>
              </tbody>
...
<script language="JavaScript">
	function frechpade(){
		...
		$("#content").load('sp/polassward.php?rech='+rech+'&crit='+crit);
	}
	function frechpade2(){
		var rech='';
		var crit=2;
	...
	function fpagepade(page,nbpage){
		if(page >=0){
			if(page == nbpage){
				...
				if(rech!='')
					$("#content").load('sp/polassward.php?page='+page+'&rech='+rech+'&crit='+crit);
				else
					$("#content").load('sp/polassward.php?page='+page);
			}
