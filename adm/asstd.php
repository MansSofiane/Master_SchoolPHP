...
$rqtc=$bdd->prepare("SELECT d.`cod_dev`,d.`dat_eff`,d.`dat_ech`,d.`pn`,d.`pt`,d.`bool`,s.`nom_sous`,s.`pnom_sous`,s.`cod_sous`,u.`agence` FROM `devisw` as d,`souscripteurw` as s,`utilisateurs` as u WHERE s.`cod_sous`=d.`cod_sous` AND d.`etat`='0' AND d.`bool`='1' AND d.`cod_prod`='6' AND s.`id_user`=u.`id_user` AND $condition ORDER BY d.`cod_dev` DESC");
$rqtc->execute();
$nbe = $rqtc->rowCount();
$nbpage=ceil($nbe/7);
//Pointeur de page
$part=$page*7;
//requete à suivre
$rqt=$bdd->prepare("SELECT d.`cod_dev`,d.`dat_eff`,d.`dat_ech`,d.`pn`,d.`pt`,d.`bool`,s.`nom_sous`,s.`pnom_sous`,s.`cod_sous`,u.`agence` FROM `devisw` as d,`souscripteurw` as s,`utilisateurs` as u WHERE s.`cod_sous`=d.`cod_sous` AND d.`etat`='0' AND d.`bool`='1' AND d.`cod_prod`='6' AND s.`id_user`=u.`id_user` AND $condition ORDER BY d.`cod_dev` DESC LIMIT $part ,7");
$rqt->execute();	
          ...
          <div class="widget-content nopadding">
			...
			<tr class="gradeX">
				  ...
				  <a href="sortie/devis3/<?php echo crypte($row_res['cod_dev']) ?>" onClick="window.open(this.href, 'Devis', 'height=600, width=800, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, directories=no, status=no'); return(false);" title="Imprimer"><i CLASS="icon-print icon-2x" style="color:#0e90d2"/></a>&nbsp;&nbsp;&nbsp;
			       ...
			       <a onClick="surprime('prod','asstd.php','<?php echo $row_res['cod_dev'];?>','<?php echo $row_res['pn'];?>')" title="Accorder"><i CLASS="icon-thumbs-up icon-2x" style="color:green"/></a>&nbsp;&nbsp;&nbsp;
				  <a onClick="rdev('prod','asstd.php','<?php echo $row_res['cod_dev'];?>')" title="Rejeter"><i CLASS="icon-thumbs-down icon-2x" style="color:red"/></a>
				  </td>
                </tr>
			<?php } ?>
              </tbody>
...
<script language="JavaScript">
	...
	function frechpade(){
		...
		$("#content").load('adm/asstd.php?rech='+rech+'&crit='+crit);
	}
	function frechpade2(){
		var rech='';
		var crit=2;
	...
	function fpagepade(page,nbpage){
		if(page >=0){
			if(page == nbpage){
				...
				if(rech!='')
					$("#content").load('adm/asstd.php?page='+page+'&rech='+rech+'&crit='+crit);
				else
					$("#content").load('adm/asstd.php?page='+page);
			}
