...
as d,`souscripteurw` as s WHERE s.`cod_sous`=d.`cod_sous`  AND d.`cod_prod`='1' AND d.`cod_formul`='3' AND s.`id_user`='$id_user' AND $condition ORDER BY d.`cod_pol` DESC");
    $rqtc->execute();
    $nbe = $rqtc->rowCount();
    $nbpage=ceil($nbe/7);
//Pointeur de page
...
as d,`souscripteurw` as s WHERE s.`cod_sous`=d.`cod_sous` AND d.`cod_prod`='1' AND d.`cod_formul`='3' AND s.`id_user`='$id_user' AND $condition ORDER BY d.`cod_pol` DESC LIMIT $part ,7");
    $rqt->execute();

}else{
//Calcule du nombre de page 
    $rqtc=$bdd->prepare("SELECT d.`cod_pol`,d.`sequence`,d.`ndat_eff`,d.`ndat_ech`,d.`pn`,d.`pt`,d.`etat`,s.`nom_sous`,s.`pnom_sous`,s.`rp_sous` FROM `policew`
 as d,`souscripteurw` as s WHERE s.`cod_sous`=d.`cod_sous` AND d.`cod_prod`='1' AND d.`cod_formul`='3' AND s.`id_user`='$id_user' ORDER BY d.`cod_pol` DESC");
    $rqtc->execute();
    $nbe = $rqtc->rowCount();
    $nbpage=ceil($nbe/7);
//Pointeur de page
...
//requete � suivre
    $rqt=$bdd->prepare("SELECT d.`cod_pol`,d.`sequence`,d.`ndat_eff`,d.`ndat_ech`,d.`pn`,d.`pt`,d.`etat`,s.`nom_sous`,s.`pnom_sous`,s.`rp_sous` FROM `policew`
as d,`souscripteurw` as s WHERE s.`cod_sous`=d.`cod_sous` AND d.`cod_prod`='1' AND d.`cod_formul`='3' AND s.`id_user`='$id_user' ORDER BY d.`cod_pol` DESC LIMIT $part ,7");
    $rqt->execute();
    $nb = $rqt->execute();
}
?>
...
<div class="widget-box">
            ...
            while ($row_res=$rqt->fetch()){  ?>
                ...
                <tr class="gradeX">
                    ...
                    <td>&nbsp;
                        ...
                        <a href="sortie/p-couple/<?php echo crypte($row_res['cod_pol']) ?>" onClick="window.open(this.href, 'Devis', 'height=600, width=800, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, directories=no, status=no'); return(false);" title="Imprimer"><i CLASS="icon-print icon-2x" style="color:#0e90d2"/></a>
                        <?php if($row_res['etat']==0){ ?>
                            &nbsp;&nbsp;&nbsp;
                        <a onClick="vavvoy('<?php echo $row_res['cod_pol'];?>','<?php echo $row_res['ndat_eff'];?>','polassvoycpl.php')" title="Avenant"><i CLASS="icon-edit icon-2x " style="color:#BA1E20" /></a>
                        <a onClick="lav('<?php echo $row_res['cod_pol'];?>','avpolvoycpl.php')" title="Liste-Avenants"><i CLASS="icon-list-alt icon-2x" style="color:black"/></a>
                        &nbsp;&nbsp;&nbsp;
                            <a onClick="DemandeResiliation('<?php echo $row_res['cod_pol'];?>','<?php echo $row_res['ndat_eff'];?>','polassvoycpl.php')" title="Demande Resiliation"><i CLASS="icon-reply icon-2x " style="color: #000" /></a>
                            <a onClick="lav('<?php echo $row_res['cod_pol'];?>','dempolvoycpl.php')" title="Liste-demande"><i CLASS="icon-list-alt icon-2x" style="color:green"/></a>
                            <?php } ?>
                        &nbsp;&nbsp;&nbsp;
                    </td>
                </tr>
    ...
    <div class="widget-title" align="center">
        <h5>Visualisation-Contrats-Voyage-Couple</h5>
...
<script language="JavaScript">
    ...
    function frechpade(){
        ...
        $("#content").load('produit/polassvoycpl.php?rech='+rech+'&crit='+crit);
    }
    function frechpade2(){
        var rech='';
        var crit=2;
    ...
    function fpagepade(page,nbpage){
        if(page >=0){
            if(page == nbpage){
                ...
                if(rech!='')
                    $("#content").load('produit/polassvoycpl.php?page='+page+'&rech='+rech+'&crit='+crit);
                else
                    $("#content").load('produit/polassvoycpl.php?page='+page);
            }
    ...
    function frechpade(){
        var rech=document.getElementById("nsouspade").value;
        $("#content").load('produit/polassvoycpl.php?rech='+rech);
    }
    function fpagepade(page,nbpage){
        if(page >=0){
            if(page == nbpage){
                alert("Vous ete a la derniere page!");
            }else{$("#content").load('produit/polassvoycpl.php?page='+page);}
        }else{alert("Vous ete en premiere page !");}
    }
    */
    function ndev(){
