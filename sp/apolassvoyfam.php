...
as d,`souscripteurw` as s, utilisateurs as u WHERE s.`cod_sous`=d.`cod_sous`  AND d.`cod_prod`='1'  and d.cod_formul='4' AND s.id_user=u.id_user AND $condition ORDER BY d.`cod_pol` DESC");
    $rqtc->execute();
    $nbe = $rqtc->rowCount();
    $nbpage=ceil($nbe/7);
//Pointeur de page
...
as d,`souscripteurw` as s, utilisateurs as u WHERE s.`cod_sous`=d.`cod_sous` AND d.`cod_prod`='1'  and d.cod_formul='4' AND s.id_user=u.id_user AND $condition ORDER BY d.`cod_pol` DESC LIMIT $part ,7");
    $rqt->execute();
            ...
            while ($row_res=$rqt->fetch()){  ?>
                ...
                <tr class="gradeX">
                    ...
                    <td>&nbsp;
                        ...
                        &nbsp;&nbsp;&nbsp;
                        <a onClick="slav('<?php echo $row_res['cod_pol'];?>','avpolvoyfam.php')" title="Liste-Avenants"><i CLASS="icon-list-alt icon-2x" style="color:black"/></a>
                    </td>
                </tr>
            <?php } ?>
            </tbody>
...
<script language="JavaScript">
    function frechpade(){
        ...
        $("#content").load('sp/apolassvoyfam.php?rech='+rech+'&crit='+crit);
    }
    function frechpade2(){
        var rech='';
        var crit=2;
    ...
    function fpagepade(page,nbpage){
        if(page >=0){
            if(page == nbpage){
                ...
                if(rech!='')
                    $("#content").load('sp/apolassvoyfam.php?page='+page+'&rech='+rech+'&crit='+crit);
                else
                    $("#content").load('sp/apolassvoyfam.php?page='+page);
            }
