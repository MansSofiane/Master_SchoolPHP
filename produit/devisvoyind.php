...
$condition AND p.`cod_per`=d.`cod_per` AND d.`cod_formul`='2' ORDER BY d.`cod_dev` DESC");
    $rqtc->execute();
    $nbe = $rqtc->rowCount();
    $nbpage=ceil($nbe/7);
//Pointeur de page
...
`devisw` as d,`souscripteurw` as s, `periode` as p WHERE s.`cod_sous`=d.`cod_sous` AND d.`etat`='0' AND d.`cod_prod`='1'  AND s.`id_user`='$id_user' AND
$condition AND p.`cod_per`=d.`cod_per` AND d.`cod_formul`='2' ORDER BY d.`cod_dev` DESC LIMIT $part ,7");
    $rqt->execute();

}else {
//Calcule du nombre de page
    $rqtc = $bdd->prepare("SELECT d.`cod_dev`,d.`cod_per`,d.`dat_eff`,d.`dat_ech`,d.`pn`,d.`pt`,d.`bool`,s.`nom_sous`,s.`pnom_sous`,p.`lib_per`,s.`rp_sous` FROM
`devisw` as d,`souscripteurw` as s, `periode` as p WHERE s.`cod_sous`=d.`cod_sous` AND d.`etat`='0' AND d.`cod_prod`='1'  AND s.`id_user`='$id_user' AND p.`cod_per`=d.`cod_per` AND d.`cod_formul`='2' ORDER BY
d.`cod_dev` DESC");
    $rqtc->execute();
    $nbe = $rqtc->rowCount();
    $nbpage = ceil($nbe / 7);
...
//requete à suivre
    $rqt = $bdd->prepare("SELECT d.`cod_dev`, d.`cod_per`,d.`dat_eff`,d.`dat_ech`,d.`pn`,d.`pt`,d.`bool`,s.`nom_sous`,s.`pnom_sous`,p.`lib_per`,s.`rp_sous` FROM
`devisw` as d,`souscripteurw` as s, `periode` as p WHERE s.`cod_sous`=d.`cod_sous` AND d.`etat`='0' AND d.`cod_prod`='1'  AND s.`id_user`='$id_user' AND
p.`cod_per`=d.`cod_per` AND d.`cod_formul`='2'  ORDER BY
d.`cod_dev` DESC LIMIT $part ,7");
    ...
    $rqtt = $bdd->prepare("SELECT d.`cod_dev`, d.`cod_per`,d.`dat_eff`,d.`dat_ech`,d.`pn`,d.`pt`,d.`bool`,s.`nom_sous`,s.`pnom_sous`,p.`lib_per`,s.`rp_sous` FROM
...
d.`cod_dev` AND d.`cod_formul`='2' DESC LIMIT $part ,7");
    $rqtt->execute();
...
<script language="JavaScript">
    function frechpade(){
        ...
        $("#content").load('produit/devisvoyind.php?rech='+rech+'&crit='+crit);
    }
    function frechpade2(){
        var rech='';
        var crit=2;
    ...
    function fpagepade(page,nbpage){
        if(page >=0){
            if(page == nbpage){
                ...
                if(rech!='')
                    $("#content").load('produit/devisvoyind.php?page='+page+'&rech='+rech+'&crit='+crit);
                else
                    $("#content").load('produit/devisvoyind.php?page='+page);
            }
        }else{alert("Vous ete en premiere page !");}
    }
   /* function frechdade(){
        var rech=document.getElementById("nsousdtd").value;
        $("#content").load('produit/devisIMMO.php?rech='+rech);
    }
    function fpagedade(page,nbpage){
        if(page >=0){
            if(page == nbpage){
                alert("Vous ete a la derniere page!");
            }else{$("#content").load('produit/devisIMMO.php?page='+page);}
        }else{alert("Vous ete en premiere page !");}
    }
    */
