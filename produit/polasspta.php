...
//Calcule du nombre de page 
    $rqtc=$bdd->prepare("SELECT d.`cod_pol`,YEAR(d.`dat_val`) as annee,d.`cod_prod`,d.`sequence`,d.`ndat_eff`,d.`ndat_ech`,d.`pn`,d.`pt`,d.`etat`,d.mtt_reg,d.mtt_solde,s.`nom_sous`,s.`pnom_sous` FROM `policew` as d,`souscripteurw` as s WHERE s.`cod_sous`=d.`cod_sous` AND d.`cod_prod`='10' AND s.`id_user` in (select id_user from utilisateurs where agence='$agence')   AND $condition ORDER BY d.`cod_pol` DESC");
    $rqtc->execute();
    $nbe = $rqtc->rowCount();
    $nbpage=ceil($nbe/7);
//Pointeur de page
    $part=$page*7;
//requete � suivre
    $rqt=$bdd->prepare("SELECT d.`cod_pol`,YEAR(d.`dat_val`) as annee,d.`cod_prod`,d.`sequence`,d.`ndat_eff`,d.`ndat_ech`,d.`pn`,d.`pt`,d.`etat`,d.mtt_reg,d.mtt_solde,s.`nom_sous`,s.`pnom_sous` FROM `policew` as d,`souscripteurw` as s WHERE s.`cod_sous`=d.`cod_sous` AND d.`cod_prod`='10' AND s.`id_user` in (select id_user from utilisateurs where agence='$agence')  AND $condition ORDER BY d.`cod_pol` DESC LIMIT $part ,7");
    $rqt->execute();

}else{
//Calcule du nombre de page 
    $rqtc=$bdd->prepare("SELECT d.`cod_pol`,YEAR(d.`dat_val`) as annee,d.`cod_prod`,d.`sequence`,d.`ndat_eff`,d.`ndat_ech`,d.`pn`,d.`pt`,d.`etat`,d.mtt_reg,d.mtt_solde,s.`nom_sous`,s.`pnom_sous` FROM `policew` as d,`souscripteurw` as s WHERE s.`cod_sous`=d.`cod_sous` AND d.`cod_prod`='10' AND s.`id_user` in (select id_user from utilisateurs where agence='$agence')  ORDER BY d.`cod_pol` DESC");
    $rqtc->execute();
    $nbe = $rqtc->rowCount();
    $nbpage=ceil($nbe/7);
//Pointeur de page
    $part=$page*7;
//requete � suivre
    $rqt=$bdd->prepare("SELECT d.`cod_pol`,YEAR(d.`dat_val`) as annee,d.`cod_prod`,d.`sequence`,d.`ndat_eff`,d.`ndat_ech`,d.`pn`,d.`pt`,d.`etat`,d.mtt_reg,d.mtt_solde,s.`nom_sous`,s.`pnom_sous` FROM `policew` as d,`souscripteurw` as s WHERE s.`cod_sous`=d.`cod_sous` AND d.`cod_prod`='10' AND s.`id_user` in (select id_user from utilisateurs where agence='$agence')  ORDER BY d.`cod_pol` DESC LIMIT $part ,7");
    $rqt->execute();
    $nb = $rqt->execute();
}
?>
...
<div class="widget-box">
    <ul class="quick-actions">
        ...
        <li class="bg_ls"> <a onClick="devpta('<?php echo $token1; ?>')"> <i class="icon-folder-open"></i>Nouveau-Devis</a> </li>
        <li class="bg_lb"> <a onClick="Menu1('prod','asspta.php')"> <i class="icon-folder-open"></i>Visualiser-Devis</a></li>
        <li class="bg_lg"> <a onClick="Menu1('prod','polasspta.php')"> <i class="icon-folder-open"></i>Visualiser-Contrats</a></li>
            ...
            while ($row_res=$rqt->fetch()){  ?>
                ...
                <tr class="gradeX">
                    ...
                    <td>&nbsp;
                        ...
                        <a href="sortie/contrat9/<?php echo crypte($row_res['cod_pol']) ?>" onClick="window.open(this.href, 'Devis', 'height=600, width=800, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, directories=no, status=no'); return(false);" title="Imprimer"><i CLASS="icon-print icon-2x" style="color:#0e90d2"/></a>

                        <?php if ($row_res['mtt_solde']>0){ ?>
                            <a onClick="reglement('<?php echo $row_res['cod_pol'];?>','0','<?php echo $row_res['cod_pol'];?>','polasspta.php')" title="Regler"><i CLASS="icon-money icon-2x" style="color:green"  /></a>&nbsp;&nbsp;&nbsp;
                        <?php }?>
                        <a onClick="lavis('<?php echo $row_res['cod_pol'];?>','avispolasspta.php','<?php echo $row_res['sequence'];?>')" title="Liste-Avis-recette"><i CLASS="icon-list icon-2x" style="color:lightseagreen"/></a>
...
<script language="JavaScript">
    function frechdade(){
        ...
        $("#content").load("produit/polasspta.php?rech="+rech+"&crit="+crit);
    }
    function frechdade2(){
        var rech='';
        var crit=2;
    ...
    function fpagedade(page,nbpage){
        if(page >=0){
            if(page == nbpage){
                ...
                if(rech!='')
                    $("#content").load("produit/polasspta.php?page="+page+"&rech="+rech+"&crit="+crit);
                else
                    $("#content").load("produit/polasspta.php?page="+page);
            }
