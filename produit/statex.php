...
?>
  ...
  <div id="content-header">
    <div id="breadcrumb"> <a class="tip-bottom"><i class="icon-signal"></i> Statistiques</a><a class="current">Production-Excel</a></div>
  </div>
  ...
  <div class="widget-box">
            ...
            <ul class="quick-actions">
			  <li class="bg_lh"> <a onClick="prod('1','<?php echo $id_user; ?>')"> <i class="icon-bar-chart"></i>A-Voyage</a> </li>
			  <li class="bg_lb"> <a onClick="prod('6','<?php echo $id_user; ?>')"> <i class="icon-bar-chart"></i>TD</a> </li>
			  <li class="bg_ly"> <a onClick="prod('7','<?php echo $id_user; ?>')"> <i class="icon-bar-chart"></i>ADE</a> </li>
			  <li class="bg_lg"> <a onClick="prod('2','<?php echo $id_user; ?>')"> <i class="icon-bar-chart"></i>I-Accident</a> </li>
			  <li class="bg_lo"> <a onClick="prod('5','<?php echo $id_user; ?>')"> <i class="icon-bar-chart"></i>C-Sein (Warda)</a> </li>
			</ul>
</div>
...
function prod(code,user) {
	...
	if (verifdatestat(date1) && verifdatestat(date2)) {
		...
		window.open('excel/Excel/' + user + '/' + d1 + '/' + d2+ '/' + code,"Generation Excel","menubar=no, status=no, scrollbars=yes, menubar=no, width=600, height=230");
